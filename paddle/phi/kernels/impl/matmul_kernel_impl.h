...
DispatchMatmulFP8Kernel(const Context& ctx,
                       ...
                       bool transpose_x,
                       bool transpose_y) {
  if (x.dtype() != DataType::FLOAT8_E4M3FN || y.dtype() != DataType::FLOAT8_E4M3FN) {
    PADDLE_THROW(phi::errors::InvalidArgument(
      "float8 matmul needs input x and y be float8_e4m3fn"));
  }

  PADDLE_ENFORCE_EQ(x_dims.size(), 2, "mat x for matmul fp8 just support 2-dim tensor");
  PADDLE_ENFORCE_EQ(y_dims.size(), 2, "mat y for matmul fp8 just support 2-dim tensor");
  PADDLE_ENFORCE_EQ(x_dims[1], y_dims[0], "x_dims[1] needs to equal to y_dims[0]");
